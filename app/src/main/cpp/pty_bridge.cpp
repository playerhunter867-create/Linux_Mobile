...
Java_org_linox_mobile_PtySession_nativeStart(
    ...
    if (pid == 0) {
        ...
        for (auto& value : args) argv.push_back(const_cast<char*>(value.c_str()));
        argv.push_back(nullptr);
        std::vector<char*> envp;
        envp.reserve(envs.size() + 1);
        for (auto& value : envs) envp.push_back(const_cast<char*>(value.c_str()));
...
Java_org_linox_mobile_PtySession_nativeRead(JNIEnv* env, jclass, jlong handle, jbyteArray out) {
    ...
    if (result > 0) env->SetByteArrayRegion(out, 0, static_cast<jsize>(result), buffer.data());
    return static_cast<jint>(result);
}

extern "C" JNIEXPORT jint JNICALL
Java_org_linox_mobile_PtySession_nativeWrite(JNIEnv* env, jclass, jlong handle, jbyteArray data) {
    auto* session = reinterpret_cast<Session*>(handle);
    ...
    while (written < static_cast<size_t>(size)) {
        const int master = session->master.load();
        if (master < 0) return written > 0 ? static_cast<jint>(written) : -1;
        const ssize_t n = write(master, buffer.data() + written, size - written);
        if (n > 0) { written += static_cast<size_t>(n); continue; }
        if (n < 0 && errno == EINTR) continue;
        return written > 0 ? static_cast<jint>(written) : -1;
    }
    return static_cast<jint>(written);
}

extern "C" JNIEXPORT void JNICALL
Java_org_linox_mobile_PtySession_nativeResize(JNIEnv*, jclass, jlong handle, jint rows, jint cols) {
    auto* session = reinterpret_cast<Session*>(handle);
...
extern "C" JNIEXPORT void JNICALL
Java_org_linox_mobile_PtySession_nativeSignal(JNIEnv*, jclass, jlong handle, jint sig) {
    auto* session = reinterpret_cast<Session*>(handle);
    if (session && session->pid.load() > 0) kill(-session->pid.load(), sig);
}

extern "C" JNIEXPORT jint JNICALL
Java_org_linox_mobile_PtySession_nativeClose(JNIEnv*, jclass, jlong handle) {
    auto* session = reinterpret_cast<Session*>(handle);
    if (!session) return -1;
    ...
    // thread. The tiny Session object is intentionally retained until process
    // death, avoiding a JNI use-after-free race on PTY shutdown.
    return 0;
}
