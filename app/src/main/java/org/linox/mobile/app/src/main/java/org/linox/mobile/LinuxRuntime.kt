...
class LinuxRuntime(private val context: Context) {
    ...
    init {
        installLayout()
        val saved = context.getSharedPreferences("linox", Context.MODE_PRIVATE).getString("active_rootfs", null)
        if (!saved.isNullOrBlank()) {
            val candidate = File(saved)
            if (candidate.isDirectory) activeRootfsFile = candidate.canonicalFile
        }
    }

    fun installLayout(): File {
        runtimeDir.mkdirs(); rootfs.mkdirs(); home.mkdirs(); tmp.mkdirs()
        return runtimeDir
    }

    fun isLinuxReady(): Boolean =
        proot.isFile && proot.canExecute() &&
            File(activeRootfsFile, "bin/sh").isFile && File(activeRootfsFile, "etc").isDirectory

    fun status(): String = when {
        isLinuxReady() -> "Linux userspace: READY"
        proot.isFile && File(activeRootfsFile, "bin/sh").isFile -> "Linux userspace: incomplete"
    ...
    fun activateRootfs(path: File) {
        ...
        context.getSharedPreferences("linox", Context.MODE_PRIVATE).edit().putString("active_rootfs", activeRootfsFile.absolutePath).apply()
        installLinOxCommands()
    }
    fun resetToDefaultRootfs() {
        activeRootfsFile = rootfs.canonicalFile
        context.getSharedPreferences("linox", Context.MODE_PRIVATE).edit().remove("active_rootfs").apply()
    }
    ...
    fun installProot(source: Uri) {
        ...
        proot.setReadable(true, true); proot.setWritable(true, true); proot.setExecutable(true, true)
    }
    ...
    fun installRootfsTarGz(source: Uri, onProgress: (String) -> Unit = {}) {
        ...
        context.contentResolver.openInputStream(source).use { raw ->
            ...
            TarArchiveInputStream(GZIPInputStream(raw.buffered())).use { tar ->
                var entry = tar.nextTarEntry; var count = 0
                while (entry != null) {
                    ...
                    when {
                        ...
                        entry.isFile -> {
                            ...
                            target.setExecutable((entry.mode and 0b001_001_001) != 0, false)
                        }
                    }
                    count++; if (count % 500 == 0) onProgress("Extracted $count files…")
                    entry = tar.nextTarEntry
                }
            }
        }
        require(File(staging, "bin/sh").isFile) { "Invalid rootfs: /bin/sh was not found" }
    ...
    /** Adds LinOx-specific commands. `nano` opens the native LinOx editor for the selected file. */
...
""")
    ...
    private fun safeTarget(base: File, name: String): File {
        val target = File(base, name).canonicalFile; val root = base.canonicalFile
        require(target.path == root.path || target.path.startsWith(root.path + File.separator)) { "Unsafe archive entry: $name" }
        return target
    }


    fun startInteractivePty(onText: (String) -> Unit): PtySession {
        check(isLinuxReady()) { "Install PRoot and an ARM64 Linux rootfs first." }
        installLinOxCommands()
        val env = linkedMapOf(
            "HOME" to "/root", "TERM" to "xterm-256color", "TMPDIR" to "/tmp",
            "LANG" to "C.UTF-8", "PROOT_NO_SECCOMP" to "1", "LINOX_ANDROID_HOME" to home.absolutePath,
            "PATH" to "/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin:/bin:/sbin"
        )
        val args = listOf("--kill-on-exit","--link2symlink","-0","-r",activeRootfsFile.absolutePath,
            "-b","/dev","-b","/proc","-b","/sys",
            "-b","/system/bin:/android-bin","-b",home.absolutePath+":/root","-b",tmp.absolutePath+":/tmp",
            "-w","/root","/bin/bash","-l")
        val p = PtySession.start(proot.absolutePath,args,env,home.absolutePath)
        p.readLoop(onText,{})
        return p
    }
    ...
    fun execute(command: String, timeoutSeconds: Long = 30): CommandResult {
        ...
        val args = if (linux) listOf(
            proot.absolutePath, "--kill-on-exit", "--link2symlink", "-0", "-r", activeRootfsFile.absolutePath,
            "-b", "/dev", "-b", "/proc", "-b", "/sys",
            "-b", "/system/bin:/android-bin", "-b", home.absolutePath + ":/root", "-b", tmp.absolutePath + ":/tmp",
            "-w", "/root", "/bin/sh", "-lc", command
        ) else listOf("/system/bin/sh", "-lc", command)
        return try {
            val process = ProcessBuilder(args)
                ...
                .apply {
                    ...
                    if (linux) environment()["PATH"] = "/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin:/bin:/sbin"
                }.start()
            val output = process.inputStream.bufferedReader().use { it.readText() }
            val finished = process.waitFor(timeoutSeconds, TimeUnit.SECONDS)
            if (!finished) { process.destroyForcibly(); CommandResult(output + "\n[LinOx] command timed out", 124) }
            else CommandResult(output, process.exitValue())
        } catch (e: Exception) { CommandResult("[LinOx] ${e.javaClass.simpleName}: ${e.message}", 1) }
    }
