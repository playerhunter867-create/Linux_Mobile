...
class PackageManagerActivity : AppCompatActivity() {
    ...
    companion object {
        val PRESETS = listOf(
            ...
            Preset("🛠 C / C++ dev", "build-essential, gdb, cmake, pkg-config", listOf("build-essential", "gdb", "cmake", "pkg-config")),
            Preset("🟩 Node.js dev", "nodejs + npm", listOf("nodejs", "npm")),
            Preset("🔧 Git & web", "git, curl, wget, vim, unzip", listOf("git", "curl", "wget", "vim", "unzip")),
            Preset("📦 Linux essentials", "bash, procps(ps), ca-certificates, jq, file, tree, less, zip, tar, gzip, xz", listOf(
                "bash", "procps", "ca-certificates", "jq", "file", "tree", "less", "zip", "tar", "gzip", "xz-utils"
            )),
            Preset("🌐 Network / SSH", "openssh-client, dnsutils, iproute2", listOf("openssh-client", "dnsutils", "iproute2")),
            Preset("🧰 Useful CLI", "htop, nano, rsync, grep, sed, awk, findutils, coreutils", listOf(
                "htop", "nano", "rsync", "grep", "sed", "gawk", "findutils", "coreutils"
            ))
        )
    }
    ...
    private fun buildUi() {
        ...
        root.addView(allButton, LinearLayout.LayoutParams(-1, -2).apply {
            setMargins(8, 4, 8, 12)
        })
    ...
    private fun installEverything() {
        if (!runtime.isLinuxReady()) {
            Toast.makeText(this, "Install a Linux distribution first", Toast.LENGTH_LONG).show()
            return
        }
        val packages = PRESETS.flatMap { it.packages }.distinct().joinToString(" ")
        runApt(
            "install -y $packages",
