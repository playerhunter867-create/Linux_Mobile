...
class DistroManager(private val context: Context, private val runtime: LinuxRuntime) {
    ...
    fun installed(): List<Distro> =
        CATALOG.filter { File(distroDir, "${it.id}/rootfs/bin/sh").isFile }

    fun isInstalled(distro: Distro) =
        File(distroDir, "${distro.id}/rootfs/bin/sh").isFile
    ...
    fun remove(distro: Distro) {
        ...
        if (
            runtime.activeRootfs().canonicalFile.path.startsWith(
                File(distroDir, distro.id).canonicalPath
            )
        ) {
    ...
    private fun downloadBlob(
        ...
        if (!target.isFile || !verifySha256(target, digest.substringAfter(':'))) {
            ...
            require(
                verifySha256(target, digest.substringAfter(':'))
            ...
            TarArchiveInputStream(stream).use { tar ->
                ...
                while (entry != null) {
                    ...
                    if (name.substringAfterLast('/').startsWith(".wh.")) {
                        ...
                        val victim = name.substringAfterLast('/').removePrefix(".wh.")
                        ...
                        if (victim == ".wh..opq") {
                            ...
                            File(
                                root,
                                if (base.isEmpty()) victim else "$base/$victim"
                            ).deleteRecursively()
                        }
