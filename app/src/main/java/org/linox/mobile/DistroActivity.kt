        ...
        val allTools = Button(this).apply {
            ...
            setOnClickListener {
                if (!runtime.isLinuxReady()) {
                    Toast.makeText(this@DistroActivity, "Install and activate Debian/Ubuntu first", Toast.LENGTH_LONG).show()
                } else {
                    startActivity(android.content.Intent(this@DistroActivity, PackageManagerActivity::class.java))
                }
            }
        }
        root.addView(allTools, LinearLayout.LayoutParams(-1, -2).apply { setMargins(8, 4, 8, 10) })
