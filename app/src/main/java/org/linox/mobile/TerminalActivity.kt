...
class TerminalActivity : AppCompatActivity() {
    ...
    private fun buildUi() {
        ...
        bar.addView(smallButton("Ctrl+D") {
            session?.write("\u0004")
        })
        bar.addView(smallButton("Tab") {
            session?.write("\t")
        })
        bar.addView(smallButton("Esc") {
            session?.write("\u001b")
        })
        ...
        listOf("pwd", "ls -la", "linox doctor", "python3 --version").forEach { command ->
            quickRow.addView(smallButton(command) { input.setText(command); sendInput() })
        }
        root.addView(
            HorizontalScrollView(this).apply { addView(quickRow) },
