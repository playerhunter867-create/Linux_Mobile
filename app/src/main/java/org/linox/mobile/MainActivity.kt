...
class MainActivity : AppCompatActivity() {
    ...
    private fun textView(value: String, size: Float = 14f): TextView =
        TextView(this).apply {
            text = value
            textSize = size
    ...
    private fun desktop() {
        ...
        hero.addView(textView("Linux userspace for Android • version 0.9.1", 14f).apply {
        ...
        grid.addView(actionCard("Linux Doctor", "Check shell, DNS, Python, Git and storage") {
            runDoctor()
        })
        layout.addView(grid)
    ...
    private fun code() {
        ...
        fun file(): File {
            val raw = path.text.toString().trim().ifEmpty { "main.py" }
            val relative = raw.replace('\\', '/').trimStart('/')
            require(relative.isNotBlank() && !relative.split('/').contains("..")) {
                "Invalid workspace path"
            }
            val workspace = WorkspaceManager(this, runtime).linuxWorkspaceDir().canonicalFile
            val candidate = File(workspace, relative).canonicalFile
            require(
                candidate.path == workspace.path ||
                    candidate.path.startsWith(workspace.path + File.separator)
            ) { "File must stay inside /root/workspace" }
            return candidate
        }
        ...
        run.setOnClickListener {
            ...
            val workspace = WorkspaceManager(this, runtime).linuxWorkspaceDir().canonicalFile
            val relative = f.canonicalFile.relativeTo(workspace).invariantSeparatorsPath
            val quoted = "'" + relative.replace("'", "'\\''") + "'"
            val cmd = when {
                f.name.endsWith(".py", ignoreCase = true) ->
                    "python3 /root/workspace/$quoted"
                f.name.endsWith(".sh", ignoreCase = true) ->
                    "sh /root/workspace/$quoted"
                else ->
    ...
    private fun runDoctor() {
        Thread {
            val result = if (runtime.isLinuxReady()) {
                runtime.execute(
                    "linox doctor; echo '---'; " +
                        "df -h / /root 2>/dev/null | tail -n +2"
                )
            } else {
                LinuxRuntime.CommandResult(
                    "Linux is not ready. Install PRoot and an ARM64 distro first.",
                    1
                )
            }
            runOnUiThread {
                android.app.AlertDialog.Builder(this)
                    .setTitle("LinOx Doctor")
