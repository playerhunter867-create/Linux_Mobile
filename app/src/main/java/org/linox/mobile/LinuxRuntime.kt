...
class LinuxRuntime(private val context: Context) {
    ...
    fun isLinuxReady(): Boolean {
        ...
        val shell = File(activeRootfsFile, "bin/sh")
        val etc = File(activeRootfsFile, "etc")
        return shell.isFile && etc.isDirectory
    }
    ...
    /** Installs an ARM64 Linux PRoot executable atomically after validating it. */
    fun installProot(source: Uri) {
        ...
        require(proot.canExecute()) {
            "Android refused execute permission for the PRoot file"
        }

        // Validate that the executable itself can start. This catches HTML/text
        // downloads, wrong-ABI binaries and missing dynamic loaders immediately.
    ...
    /** Installs a tar.gz rootfs atomically, preserving symlinks and hardlinks. */
                    ...
                    while (entry != null) {
                        ...
                        when {
                            ...
                            entry!!.isLink -> {
                                ...
                                require(linked.isFile) { "Broken hardlink: ${entry!!.linkName}" }
                            ...
                            entry!!.isFile -> {
                                ...
                                // Device nodes/FIFOs are intentionally omitted. /dev is
                                // supplied by the Android bind mount when PRoot starts.
                            }
                        }
                        entries[name] = target
                        count++
...
""".trimStart()
    ...
    private fun normalizeArchiveName(raw: String): String {
        ...
        require(name.isNotEmpty() || raw.isNotEmpty()) { "Invalid empty archive entry" }
    ...
    private fun createSafeSymlink(base: File, target: File, linkName: String, entryName: String) {
        ...
        val linkParent = target.parentFile ?: error("Symlink has no parent: $entryName")
        // Validate the link lexically inside the archive root. Do not call
        // canonicalFile on the symlink target: an absolute Linux link such as
        // /usr/lib must remain absolute inside the future PRoot root, not on
        // the Android host filesystem.
        val targetPath = if (linkName.startsWith('/')) {
            "/" + linkName.trimStart('/')
        } else {
            "/" + target.relativeTo(base).parent.orEmpty().replace(File.separatorChar, '/') + "/" + linkName
        }
        ...
        for (part in parts) {
            ...
            require(depth >= 0) { "Unsafe symlink target: $entryName -> $linkName" }
        }
        java.nio.file.Files.createSymbolicLink(target.toPath(), java.nio.file.Paths.get(linkName))
    }

    private fun deleteAny(file: File) {
        if (file.exists() || java.nio.file.Files.isSymbolicLink(file.toPath())) file.deleteRecursively()
    }
    ...
    private fun isSafeRootfsPath(candidate: File): Boolean {
        ...
        return candidate.path == runtime.path || candidate.path.startsWith(runtime.path + File.separator)
    }
        ...
        return runCatching {
            RandomAccessFile(file, "r").use { raf ->
                ...
                ident[0] == 0x7f.toByte() &&
                    ...
                    ident[4].toInt() == 2 && // ELF64
                    ident[5].toInt() == 1 && // little-endian
                    (ident[18].toInt() and 0xff) == 183 && // EM_AARCH64
                    (ident[19].toInt() and 0xff) == 0
            }
        }.getOrDefault(false)
    }
}
