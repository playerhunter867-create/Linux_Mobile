...
class WorkspaceActivity : AppCompatActivity() {
    ...
    private fun buildUi() {
        ...
        root.addView(
            TextView(this).apply {
                text =
                    "Linux side: /root/workspace  " +
                        "(also reachable from the terminal and Code editor)"
                textSize = 13f
        ...
        val pull = Button(this).apply {
            text = "⬇ Pull to Linux"
            setOnClickListener { sync(pull = true) }
        }

        val push = Button(this).apply {
            text = "⬆ Push to Android"
            setOnClickListener { sync(pull = false) }
        }

        syncRow.addView(pull)
        syncRow.addView(push)
    ...
    private fun refreshFolderLabel() {
        ...
        folderLabel.text =
            if (uri != null) {
                ...
                "No Android folder chosen yet."
            }
    }

    private fun sync(pull: Boolean) {
        val uri: Uri = manager.savedFolderUri() ?: run {
            Toast.makeText(
                this,
                "Choose an Android folder first",
                Toast.LENGTH_SHORT
            ).show()
            return
        }
        ...
        executor.execute {
            try {
                val result =
                    if (pull) {
                        manager.pullFromAndroid(uri) { m ->
                            runOnUiThread {
                                output.text = m
                            }
                        }
                    } else {
                        manager.pushToAndroid(uri) { m ->
                            runOnUiThread {
                                output.text = m
                            }
                        }
                ...
                runOnUiThread {
                    busyBar.visibility = ProgressBar.INVISIBLE
                    output.text = "Sync failed: ${e.message}"
                }
            }
        }
    }
