...
class PtySession private constructor(private val handle: Long) : AutoCloseable {
    ...
    companion object {
        ...
        fun start(
            ...
            require(h != 0L) {
                "Unable to start PTY. Check that PRoot is executable and the selected rootfs is valid."
            }
            return PtySession(h)
        }
        ...
        @JvmStatic
        private external fun nativeRead(handle: Long, out: ByteArray): Int
        ...
        @JvmStatic
        private external fun nativeSignal(handle: Long, sig: Int)
    ...
    fun write(text: String) {
        if (closed.get() || text.isEmpty()) return
